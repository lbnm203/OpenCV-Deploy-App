...
if choice == 'WaterShed':
    ...
    def img_training(idx1, idx2):
        ...
        col1.markdown('   <p style="text-indent: 130px;"> <span style = "font-size:25px;"> Ảnh gốc</span>', unsafe_allow_html=True)
        ...
        # In ảnh
        col1.image(img_ori1)
        col1.markdown(f'   <p style="text-indent: 130px;"> <span style = " font-size:25px;"> {list_image[idx1]}</span>', unsafe_allow_html=True)
        
        col1.image(img_ori2)
        col1.markdown(f'   <p style="text-indent: 130px;"> <span style = " font-size:25px;"> {list_image[idx2]} </span>', unsafe_allow_html=True)
        
        col2.markdown('   <p style="text-indent: 90px;"> <span style = " font-size:25px;"> Ảnh Ground truth</span>', unsafe_allow_html=True)

        col2.image(img_gt1)
        col2.markdown(f'   <p style="text-indent: 130px;"> <span style = " font-size:25px;">{list_image[idx1]}</span>', unsafe_allow_html=True)

        col2.image(img_gt2)
        col2.markdown(f'   <p style="text-indent: 130px;"> <span style = " font-size:25px;"> {list_image[idx2]}</span>', unsafe_allow_html=True)
    ...
    def plot_dice(lst_dice_1, lst_dice_2, thresh):
        ...
        axis2.set_title('Biểu đồ Dice Coeffiecent theo Threshold và Kernel của Ảnh 2')
        axis2.legend()
    ...
    def result_of_test(best_kernel, best_thresh):
        watershed_res, ground_truth, ret, dice = apply_best_params(best_kernel, best_thresh, 2, 4)
        st.markdown(f"### 4. Kết quả thí nghiệm")
        cc1, cc2 = st.columns(2)
        #Plot Test1
        cc1.image(ground_truth[0])
        cc1.markdown(f'   <p style="text-indent: 90px;"> <span style = "font-size:25;"> Ground Truth {list_image[2]}  </span>', unsafe_allow_html=True)
        cc2.image(watershed_res[0])
        cc2.markdown(f'   <p style="text-indent: 90px;"> <span style = "font-size:25;"> IoU = {ret[1]:.2f}, Dice Coeffiecent = {dice[1]:.2f} </span>', unsafe_allow_html=True)
        
        #Plot Test2t
        cc1.image(ground_truth[1])
        cc1.markdown(f'   <p style="text-indent: 90px;"> <span style = "font-size:25;"> Ground Truth {list_image[3]} </span>', unsafe_allow_html=True)
